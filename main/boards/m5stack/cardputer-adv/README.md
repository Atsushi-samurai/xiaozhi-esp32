...
- **Wi-Fiを再設定するには:** 待機画面（idle）で、左下の `Ctrl` を押したまま `W` を**約4秒**（3秒の長押し成立後、750msの確認表示が終わるまで）押し続けます。Cardputer本体で周囲のSSIDを検索して一覧表示します。`;`／`.` で選択し、`Enter`、パスワードを入力して再度 `Enter` を押すと接続します。スマホ・ブラウザ・AP/captive portalは不要です。途中でどちらかのキーを離すと取り消されます。会話中・接続中には動作しません。
