...
std::string Ota::GetDevicePersonaApiUrl() {
    std::string ota_url = GetCheckVersionUrl();
    const std::string ota_path = "/ota/";
    size_t ota_path_pos = ota_url.rfind(ota_path);
    if (ota_path_pos == std::string::npos) {
        ESP_LOGE(TAG, "OTA URL does not contain /ota/: %s", ota_url.c_str());
        return "";
    }
    return ota_url.substr(0, ota_path_pos) + "/device/personas";
}
...
esp_err_t Ota::CheckVersion() {
    ...
    cJSON* device_persona = cJSON_GetObjectItem(root, "device_persona");
    if (cJSON_IsObject(device_persona)) {
        cJSON* token = cJSON_GetObjectItem(device_persona, "token");
        if (cJSON_IsString(token)) {
            device_persona_token_ = token->valuestring;
        }
    }
